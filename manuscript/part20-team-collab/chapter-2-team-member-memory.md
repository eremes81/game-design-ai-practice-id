---
title: "20.2 Memori per Anggota Tim — Memisahkan Kompartemen Pengguna dan Kompartemen Bersama"
part: 20
chapter: 2
status: v3
version: v3
...
Keempat kompartemen pribadi saya warnai biru, dan satu kompartemen bersama saya warnai oranye. Alasan warnanya berbeda adalah karena aturan aksesnya berbeda. Kompartemen biru hanya dibuka oleh pemiliknya dan direktur, sedangkan kompartemen oranye dibuka semua orang. Insiden 0.6 detik terjadi karena anggota tim B menyebut nilai eksperimen yang seharusnya ditulis di kompartemen birunya sendiri begitu saja sebagai "memori" tanpa membedakan warna, lalu memperlakukannya seolah keputusan bersama. Jika kompartemen dipisahkan secara fisik — yaitu jika direktorinya dipisah — setidaknya muncul petunjuk untuk membedakan keduanya berdasarkan di mana sesuatu ditulis.

Inti di sini bukan dua folder, melainkan bahwa **setiap kompartemen membawa aturannya masing-masing**. Yang masuk ke `shared/` adalah keputusan perusahaan dan dibaca siapa pun. Yang masuk ke `Anggota tim B/` adalah konteks kerja orang itu dan hanya dibaca oleh dirinya dan saya. Walau angkanya sama-sama 0.6 detik, tergantung di kompartemen mana ia berada, statusnya terbelah antara "sedang dieksperimenkan" atau "sudah diputuskan".

---

## 20.2.2 Di Dalam Kompartemen Satu Orang Ada Dua Berkas

Saat membuka kompartemen per pengguna, terlihat dua berkas. `context.md` dan `notes.md`. Namanya sederhana, tetapi perannya berlawanan.
...
```mermaid
flowchart TD
    A["teammate_c notes.md<br/>(kompartemen pribadi·diperbarui harian)"] --> B{"Retrospektif mingguan<br/>gerbang promosi"}
    B -->|"Bermanfaat bagi tim + info pribadi dihapus"| C["Tinjauan anonimisasi"]
    B -->|"Konteks pribadi·emosi·konflik"| D["Tetap di kompartemen pribadi"]
...
Kriteria penilaian gerbang ada dua. **Pertama, apakah bermanfaat bagi tim.** Bukan selera pribadi atau kondisi badan hari itu. Kedua, **apakah informasi pribadinya dihapus.** Bukan "anggota tim C kembali salah di enum", melainkan hanya menyisakan fakta sebagai "tambahkan validasi urutan enum saat mengimpor sheet data". Hanya yang lolos dua gerbang itu yang menuju `shared/`. Yang tidak lolos dibiarkan apa adanya di kompartemen pribadi.

Tanpa gerbang ini, kita gagal dalam salah satu dari dua arah. Jika gerbang terlalu longgar, informasi pribadi bocor ke kompartemen bersama dan terjadi kebalikan dari insiden 0.6 detik — yaitu insiden memo pribadi terekspos ke semua orang. Jika gerbang sama sekali tidak ada, pelajaran enum dari anggota tim C terkurung di kompartemennya sendiri dan anggota tim A mengulangi kesalahan yang sama. Dengan memakai retrospektif sebagai gerbang, dua hal yang berlawanan — "kebocoran informasi pribadi" dan "pengubahan menjadi aset pembelajaran" — dapat dipisahkan dalam satu gerakan.
...
Tahap 2 saya loloskan. Pada nomor 2, baik orang maupun jalur penyampaian sudah hilang dan hanya tinggal prosedur. Dimulai dengan 4 butir, 2 butir naik ke shared, 1 butir tetap di kompartemen pribadi (cuti) dan 1 butir terpisah menuju isu portal (bug filter). Inilah wujud gerbang yang benar-benar bekerja — tidak selesai sekali coba, manusia menolak satu kali, dan tertinggal sebagai catatan apa yang dikeluarkan dan mengapa. Kalau diringkas memang terlihat rapi, tetapi jika hanya melihat ringkasan yang rapi, fakta bahwa nomor 2 nyaris naik dengan membawa nama orang di awal jadi terhapus. Risiko itulah alasan keberadaan gerbang ini.

---

## 20.2.5 Injeksi JIT — Kompartemen Mana yang Dibuka Itulah Antarmukanya

Sudah memisahkan kompartemen dan mendirikan gerbang pun, jika setiap kali berdialog kita harus membuka kompartemen secara manual, operasionalnya berat. Karena itu, kepingan terakhir adalah **kompartemen yang sesuai dengan konteks dialog terbuka secara otomatis**. Di PC saya sendiri, hal ini dilakukan oleh hook UserPromptSubmit (`inject_memory.py`). Ia hanya memilih kompartemen yang cocok dengan kalimat masukan dan menyuntikkannya ke konteks.

Aturannya sederhana. Saat mendiskusikan keputusan game, atom `shared/` terbuka. Saat menyiapkan sesi 1:1 dengan anggota tim tertentu, `context.md` orang itu + `shared` terbuka bersama. Saat menulis retrospektif kuartal, memori proyek + kompartemen direktur sendiri terbuka. Saat menulis laporan eksternal, kompartemen direktur + sebagian shared terbuka. Kompartemen mana yang dibuka itulah antarmuka dari memori.

Di sinilah pemisahan kompartemen kembali berdaya. Saat menyiapkan sesi 1:1, kompartemen pribadi anggota tim B terbuka tetapi kompartemen pribadi anggota tim C tidak — karena tidak ada kaitannya dengan dialog saat ini. Jika kompartemen tidak dipisahkan, setiap kali semuanya terbuka dan terkubur dalam derau, dan yang lebih buruk, memo pribadi orang yang tidak relevan ikut terseret keluar di tengah sesi 1:1. Pemisahan adalah keamanan sekaligus akurasi injeksi.

---

## 20.2.6 Data yang Sama, Banyak PC — Tempat Mencegah Insiden Sinkronisasi

Walau struktur kompartemen sudah tertata, masih tersisa satu jebakan terakhir. Saya berpindah-pindah antara PC rumah dan PC kantor, dan memori disinkronkan lewat folder cloud. Di sini, jika dua PC mengubah kompartemen yang sama secara bersamaan, terjadi konflik. Jika satu sisi menimpa sisi lain secara utuh, `notes.md` hari itu akan lenyap.

Penanganannya berbeda per kompartemen. `notes.md` pribadi yang sering diperbarui ditaruh di repositori yang dapat di-merge seperti git, dan saat konflik kedua sisi digabungkan. `context.md` yang stabil atau atom `shared/` jarang diperbarui, sehingga cukup dengan penguncian atau pencadangan harian. Intinya adalah mengeluarkan "perilaku sinkronisasi menimpa satu sisi" dari nilai bawaan. Insiden tersinkronnya kompartemen pribadi yang tercampur ke folder bersama akibat izin folder yang salah — itulah insiden yang paling sunyi dan paling fatal. Jika kita menandai dengan jelas tiap kompartemen termasuk ke wilayah sinkronisasi mana, insiden "tercampur" yang sejenis dengan insiden 0.6 detik dapat dicegah di pintu masuk.
...
2. Hanya kandidat yang lolos yang dipindahkan ke atom `shared/`, dan butir yang dibuang dibiarkan apa adanya di kompartemen pribadi.
3. Periksa izin folder sinkronisasi — pastikan kompartemen pribadi tidak masuk ke dalam jalur folder bersama.

**Versi Ringkas Solo**
Jika sendirian, lima folder itu berlebihan. Tulis saja `notes.md` satu setiap hari, dan simpan `context.md` di kepala. Meski begitu, gerbangnya tetap dihidupkan — saring catatan Anda sendiri seminggu sekali dengan "dari notes ini, pilih hanya satu baris yang layak dilihat lagi lain kali", maka memo yang mudah menguap dan pelajaran yang menjadi aset akan terbelah. Saat jumlah orang bertambah menjadi dua, barulah pecah kompartemennya saat itu.

---

### Poin-Poin Penting

- Lemari memori terbagi menjadi kompartemen pribadi per pengguna dan kompartemen bersama yang dibuka semua orang, dan jika dua kompartemen berbeda warna ini tidak dipaksakan lewat alat, nilai eksperimen akan menjelma menjadi keputusan.
- Di jalur menaikkan pelajaran dari kompartemen pribadi ke bersama ada gerbang bernama retrospektif, yang memisahkan pengubahan menjadi aset pembelajaran dan kebocoran informasi pribadi dalam satu gerakan.
- Injeksi JIT hanya membuka kompartemen yang sesuai dengan konteks dialog, sehingga membuat pemisahan kompartemen bekerja sekaligus sebagai keamanan dan akurasi injeksi.

### Pratinjau Bab Berikutnya

- 20.3 Membangun Portal Web — antarmuka terpadu untuk mengakses alat dan kompartemen yang tersebar dari satu layar
